...
class Stage8CICDBatch14Tests(unittest.TestCase):
    ...
    def test_batch_id_is_present_and_valid(self):
        ...
        self.assertIn("CRT-CI-052", ids)

    def test_named_floating_ref_fixtures(self):
        for name in ("stable", "release", "production", "develop", "dev", "next", "vnext"):
            line = f"    uses: acme/build-action@{name}"
            found = {rule.rule_id for rule, _ in match_rules(line, self.workflow)}
            self.assertIn("CRT-CI-052", found)
    ...
    def test_safe_controls_do_not_match(self):
        for line in (
            ...
            self.assertNotIn("CRT-CI-052", {rule.rule_id for rule, _ in match_rules(line, self.workflow)})

    def test_batch_metadata_is_ci_policy(self):
        rule = next(rule for rule in DEFAULT_DETECTION_RULES + DEFAULT_CONTEXT_RULES if rule.rule_id == "CRT-CI-052")

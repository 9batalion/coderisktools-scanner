...
class Stage8CICDBatch27Tests(unittest.TestCase):
    workflow = ".github/workflows/pr.yml"
    def test_registry(self):
        validate_rule_registry(DEFAULT_DETECTION_RULES); validate_context_rule_registry(DEFAULT_CONTEXT_RULES)
        self.assertIn("CRT-CI-064", {r.rule_id for r in DEFAULT_CONTEXT_RULES})
    def test_top_level_pull_requests_write(self):
        content="permissions:\n  pull-requests: write"
        found={m.rule.rule_id for m in match_context_rules(list(enumerate(content.splitlines(),1)),self.workflow)}
        self.assertIn("CRT-CI-064",found)
    def test_safe_scopes_do_not_match(self):
        for content in ("permissions:\n  pull-requests: read", "jobs:\n  pr:\n    permissions:\n      pull-requests: write", "permissions:\n  contents: write"):
            found={m.rule.rule_id for m in match_context_rules(list(enumerate(content.splitlines(),1)),self.workflow)}
            self.assertNotIn("CRT-CI-064",found)
    def test_metadata(self):
        r=next(r for r in DEFAULT_CONTEXT_RULES if r.rule_id=="CRT-CI-064")

...
class DetectionInventoryTests(unittest.TestCase):
    def test_authoritative_registry_reconciles_to_196(self):
        inventory = build_inventory()
        self.assertEqual(inventory["counts"]["native_rule_count"], 196)
        self.assertEqual(inventory["counts"]["line_rule_count"], 190)
        self.assertEqual(inventory["counts"]["context_rule_count"], 6)
        self.assertEqual(sum(inventory["counts"]["category"].values()), 196)
    ...
    def test_detector_ids_are_unique_and_families_are_seeded(self):
        ...
        self.assertEqual(len({item["detector_id"] for item in detectors}), 196)

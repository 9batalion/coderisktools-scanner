...
class Stage8CICDBatch35Tests(unittest.TestCase):
    ...
    def test_registry(self):
        ...
        self.assertIn("CRT-CI-071", {r.rule_id for r in DEFAULT_CONTEXT_RULES})
    def test_top_level_actions_write(self):
        found={m.rule.rule_id for m in match_context_rules(list(enumerate("permissions:\n  actions: write".splitlines(),1)),self.workflow)}
        self.assertIn("CRT-CI-071",found)
    def test_safe_scopes_do_not_match(self):
        for content in ("permissions:\n  actions: read", "jobs:\n  run:\n    permissions:\n      actions: write", "permissions:\n  contents: write"):
            ...
            self.assertNotIn("CRT-CI-071",found)
    def test_metadata(self):
        r=next(r for r in DEFAULT_CONTEXT_RULES if r.rule_id=="CRT-CI-071")

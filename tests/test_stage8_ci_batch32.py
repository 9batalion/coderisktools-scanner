...
class Stage8CICDBatch32Tests(unittest.TestCase):
    ...
    def test_registry(self):
        ...
        self.assertIn("CRT-CI-068", {r.rule_id for r in DEFAULT_CONTEXT_RULES})
    def test_top_level_attestations_write(self):
        content="permissions:\n  attestations: write"
        found={m.rule.rule_id for m in match_context_rules(list(enumerate(content.splitlines(),1)),self.workflow)}
        self.assertIn("CRT-CI-068",found)
    def test_safe_scopes_do_not_match(self):
        for content in ("permissions:\n  attestations: read", "jobs:\n  attest:\n    permissions:\n      attestations: write", "permissions:\n  contents: write"):
            found={m.rule.rule_id for m in match_context_rules(list(enumerate(content.splitlines(),1)),self.workflow)}
            self.assertNotIn("CRT-CI-068",found)
    def test_metadata(self):
        r=next(r for r in DEFAULT_CONTEXT_RULES if r.rule_id=="CRT-CI-068")

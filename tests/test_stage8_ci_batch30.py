...
class Stage8CICDBatch30Tests(unittest.TestCase):
    workflow = ".github/workflows/mutate.yml"
    def test_registry(self):
        validate_rule_registry(DEFAULT_DETECTION_RULES); validate_context_rule_registry(DEFAULT_CONTEXT_RULES)
        self.assertIn("CRT-CI-067", {r.rule_id for r in DEFAULT_CONTEXT_RULES})
    def test_top_level_workflows_write(self):
        content="permissions:\n  workflows: write"
        found={m.rule.rule_id for m in match_context_rules(list(enumerate(content.splitlines(),1)),self.workflow)}
        self.assertIn("CRT-CI-067",found)
    def test_safe_scopes_do_not_match(self):
        for content in ("permissions:\n  workflows: read", "jobs:\n  mutate:\n    permissions:\n      workflows: write", "permissions:\n  contents: write"):
            found={m.rule.rule_id for m in match_context_rules(list(enumerate(content.splitlines(),1)),self.workflow)}
            self.assertNotIn("CRT-CI-067",found)
    def test_metadata(self):
        r=next(r for r in DEFAULT_CONTEXT_RULES if r.rule_id=="CRT-CI-067")

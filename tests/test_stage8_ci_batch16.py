...
class Stage8CICDBatch16Tests(unittest.TestCase):
    ...
    def test_batch_id_is_present_and_valid(self):
        ...
        self.assertIn("CRT-CI-054", ids)

    def test_dynamic_action_ref_fixtures(self):
        for line in (
            "    uses: acme/build-action@${{ github.ref_name }}",
            "    uses: acme/build-action@${{ inputs.action_ref }}",
        ):
            found = {rule.rule_id for rule, _ in match_rules(line, self.workflow)}
            self.assertIn("CRT-CI-054", found)
    ...
    def test_safe_controls_do_not_match(self):
        for line in (
            ...
            self.assertNotIn("CRT-CI-054", {rule.rule_id for rule, _ in match_rules(line, self.workflow)})

    def test_batch_metadata_is_ci_policy(self):
        rule = next(rule for rule in DEFAULT_DETECTION_RULES + DEFAULT_CONTEXT_RULES if rule.rule_id == "CRT-CI-054")

...
DEFAULT_CONTEXT_RULES: list[ContextDetectionRule] = [
    ...
    ContextDetectionRule("GH_PERMISSIONS_TOP_LEVEL_METADATA", (r"^permissions\s*:\s*$", r"^\s*metadata\s*:\s*write\s*$"), 4, "medium", "GitHub Actions workflow grants artifact metadata write access at workflow scope", "CRT-CI-075", "ci", "high", "Scope metadata: write to the narrowly required artifact-metadata job and keep unrelated workflow jobs on minimum token permissions.", "policy", (".github/workflows/*.yml", ".github/workflows/*.yaml")),

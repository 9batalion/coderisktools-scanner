...
class Stage8CICDBatch26Tests(unittest.TestCase):
    ...
    def test_registry(self):
        ...
        self.assertIn("CRT-CI-063", {r.rule_id for r in DEFAULT_CONTEXT_RULES})
    def test_top_level_issues_write(self):
        content="permissions:\n  issues: write"
        found={m.rule.rule_id for m in match_context_rules(list(enumerate(content.splitlines(),1)),self.workflow)}
        self.assertIn("CRT-CI-063",found)
    def test_safe_scopes_do_not_match(self):
        for content in ("permissions:\n  issues: read", "jobs:\n  triage:\n    permissions:\n      issues: write", "permissions:\n  contents: write"):
            found={m.rule.rule_id for m in match_context_rules(list(enumerate(content.splitlines(),1)),self.workflow)}
            self.assertNotIn("CRT-CI-063",found)
    def test_metadata(self):
        r=next(r for r in DEFAULT_CONTEXT_RULES if r.rule_id=="CRT-CI-063")

...
PyPI (`pypi`) range matching now uses the stdlib-only bounded PEP 440 implementation. npm (`npm`) version precedence uses SemVer 2.0, and `matches_npm_range` supports the bounded range subset: exact/partial/wildcard versions, `> >= < <=`, `~`, `^`, whitespace AND and `||` OR. Other ecosystems continue to use the bounded fallback until their own comparator batch is completed.

...
class Stage8CICDBatch34Tests(unittest.TestCase):
    workflow = ".github/workflows/pages.yml"
    def test_registry(self):
        validate_rule_registry(DEFAULT_DETECTION_RULES); validate_context_rule_registry(DEFAULT_CONTEXT_RULES)
        self.assertIn("CRT-CI-070", {r.rule_id for r in DEFAULT_CONTEXT_RULES})
    def test_top_level_pages_write(self):
        found={m.rule.rule_id for m in match_context_rules(list(enumerate("permissions:\n  pages: write".splitlines(),1)),self.workflow)}
        self.assertIn("CRT-CI-070",found)
    def test_safe_scopes_do_not_match(self):
        for content in ("permissions:\n  pages: read", "jobs:\n  deploy:\n    permissions:\n      pages: write", "permissions:\n  contents: write"):
            ...
            self.assertNotIn("CRT-CI-070",found)
    def test_metadata(self):
        r=next(r for r in DEFAULT_CONTEXT_RULES if r.rule_id=="CRT-CI-070")

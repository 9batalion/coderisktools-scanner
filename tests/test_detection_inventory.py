...
class DetectionInventoryTests(unittest.TestCase):
    def test_authoritative_registry_reconciles_to_291(self):
        inventory = build_inventory()
        self.assertEqual(inventory["counts"]["native_rule_count"], 291)
        self.assertEqual(inventory["counts"]["line_rule_count"], 267)
        self.assertEqual(inventory["counts"]["context_rule_count"], 24)
        self.assertEqual(sum(inventory["counts"]["category"].values()), 291)
    ...
    def test_detector_ids_are_unique_and_families_are_seeded(self):
        ...
        self.assertEqual(len({item["detector_id"] for item in detectors}), 291)
    ...
    def test_cli_uses_canonical_checkout_registry(self):
        ...
        self.assertEqual(document["counts"]["native_rule_count"], 291)

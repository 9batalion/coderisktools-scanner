...
class Stage8CICDBatch12Tests(unittest.TestCase):
    ...
    def test_batch_id_is_present_and_valid(self):
        ...
        self.assertIn("CRT-CI-050", ids)

    def test_version_tag_fixture(self):
        line = "    uses: acme/build-action@v4"
        found = {rule.rule_id for rule, _ in match_rules(line, self.workflow)}
        self.assertIn("CRT-CI-050", found)
    ...
    def test_safe_controls_do_not_match(self):
        ...
        for line in safe:
            self.assertNotIn("CRT-CI-050", {rule.rule_id for rule, _ in match_rules(line, self.workflow)})

    def test_batch_metadata_is_ci_policy(self):
        rule = next(rule for rule in DEFAULT_DETECTION_RULES + DEFAULT_CONTEXT_RULES if rule.rule_id == "CRT-CI-050")
